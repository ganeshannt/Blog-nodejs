...
Follow.prototype.validate = async function (action) {
    ...
    let doesFollowExist = await followCollection.findOne({ wantToFollowId: this.wantToFollowId, authorId: new ObjectID(this.authorId) })
    if (action == "create") {
        if (doesFollowExist) { this.errors.push("You aleardy following this user") }
    }
    if (action == "delete") {
        if (!doesFollowExist) { this.errors.push("you are not able to unfollow the user that you were not followed") }
    }
    // should not able to follow yourself 
    if (this.wantToFollowId.equals(this.authorId)) { this.errors.push("you are not able to follow yourself") }
...
Follow.prototype.delete = function () {
    return new Promise(async (resolve, reject) => {
        ...
        if (!this.errors.length) {
            await followCollection.deleteOne({ wantToFollowId: this.wantToFollowId, authorId: new ObjectID(this.authorId) })
            resolve()
        } else {
...
Follow.countFollowerByAuthor = function(id){
    return new Promise(async (resolve,reject)=>{
        let followerCount = await followCollection.countDocuments({wantToFollowId : id})
